...
/**
 * Tests for the InputValidatedServlet class.
 *
 * @author Dominik Schadow
 */
class InputValidatedServletTest {
    @Mock
    ...
    private InputValidatedServlet servlet;
    ...
    void doPost_setsContentTypeToHtml() throws Exception {
        when(request.getParameter("inputValidatedName")).thenReturn("TestName");
    ...
    void doPost_outputContainsTitle() throws Exception {
        ...
        String output = stringWriter.toString();
        assertTrue(output.contains("<title>Cross-Site Scripting (XSS) - Input Validation</title>"));
    }
    ...
    void doPost_outputContainsHeading() throws Exception {
        ...
        String output = stringWriter.toString();
        assertTrue(output.contains("<h1>Cross-Site Scripting (XSS) - Input Validation</h1>"));
    }

    @Test
    void doPost_withNullName_outputContainsNull() throws Exception {
        when(request.getParameter("inputValidatedName")).thenReturn(null);
        when(response.getWriter()).thenReturn(printWriter);
    ...
    void doPost_withEmptyName_outputContainsEmptyBrackets() throws Exception {
        when(request.getParameter("inputValidatedName")).thenReturn("");
        when(response.getWriter()).thenReturn(printWriter);
    ...
    void doPost_withScriptTag_outputContainsScriptTag() throws Exception {
        ...
        when(request.getParameter("inputValidatedName")).thenReturn(maliciousInput);
        when(response.getWriter()).thenReturn(printWriter);
    ...
    void doPost_withSpecialCharacters_outputContainsSpecialCharacters() throws Exception {
        ...
        when(request.getParameter("inputValidatedName")).thenReturn(specialChars);
        when(response.getWriter()).thenReturn(printWriter);
